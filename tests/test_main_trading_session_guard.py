"""main.py trading-session-check / live-approve session guard (mock only)."""

from __future__ import annotations

import json
from datetime import datetime
from pathlib import Path
from unittest.mock import MagicMock, patch
from zoneinfo import ZoneInfo

import pytest

import main as main_mod
from deepsignal.live_trading.trading_session import TradingSessionResult


def test_trading_session_check_cli_open() -> None:
    rc = main_mod.main(
        [
            "trading-session-check",
            "--now",
            "2026-05-15T10:00:00+09:00",
        ]
    )
    assert rc == 0


def test_trading_session_check_cli_closed() -> None:
    rc = main_mod.main(
        [
            "trading-session-check",
            "--now",
            "2026-05-15T08:00:00+09:00",
        ]
    )
    assert rc == 1


def _kr_plan(tmp_path: Path) -> Path:
    d = {
        "date": "2026-05-15",
        "status": "PENDING_APPROVAL",
        "approval_required": True,
        "dry_run": True,
        "capital": 1_000_000.0,
        "investable_cash": 1_000_000.0,
        "cash_buffer": 0.0,
        "currency": "KRW",
        "orders": [
            {
                "symbol": "005930",
                "side": "BUY",
                "target_weight": 1.0,
                "target_value": 70_000.0,
                "estimated_price": 70_000.0,
                "estimated_qty": 1,
                "estimated_order_value": 70_000.0,
                "reason": "t",
                "warnings": [],
            }
        ],
        "warnings": [],
    }
    p = tmp_path / "live_order_plan_20260515.json"
    p.write_text(json.dumps(d), encoding="utf-8")
    return p


def _mock_session() -> MagicMock:
    class TokResp:
        def raise_for_status(self) -> None:
            return

        def json(self) -> dict:
            return {"access_token": "tok", "expires_in": 600}

    class OrderResp:
        status_code = 200
        text = '{"rt_cd":"0","msg1":"ok","output":{"ODNO":"1"}}'

        def json(self) -> dict:
            return {"rt_cd": "0", "msg1": "ok", "output": {"ODNO": "1"}}

    session = MagicMock()

    def post(url: str, **kwargs: object) -> TokResp | OrderResp:
        if "oauth2/tokenP" in url:
            return TokResp()
        if "order-cash" in url:
            return OrderResp()
        raise AssertionError(url)

    session.post.side_effect = post
    return session


def test_live_approve_session_closed_no_post(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("KIS_APP_KEY", "k")
    monkeypatch.setenv("KIS_APP_SECRET", "s")
    monkeypatch.setenv("KIS_ACCOUNT_NO", "12345678")
    monkeypatch.setenv("KIS_ACCOUNT_PRODUCT_CODE", "01")
    monkeypatch.setenv("KIS_ENV", "live")
    p = _kr_plan(tmp_path)
    closed = TradingSessionResult(
        is_open=False,
        reason="outside regular trading hours",
        market="KR",
        now="2026-05-15T08:00:00+09:00",
        timezone="Asia/Seoul",
        session_open="09:00",
        session_close="15:30",
    )

    session = _mock_session()

    def _closed(*args: object, **kwargs: object) -> TradingSessionResult:
        return closed

    with patch("deepsignal.live_trading.kis_broker.requests.Session", return_value=session):
        with patch("deepsignal.live_trading.trading_session.is_trading_session_open", side_effect=_closed):
            rc = main_mod.main(
                [
                    "live-approve",
                    "--broker",
                    "kis",
                    "--plan",
                    str(p),
                    "--approved",
                    "--execute",
                    "--allow-live-env",
                    "--final-confirm",
                    "I_UNDERSTAND_REAL_ORDER",
                    "--allow-symbol",
                    "005930",
                    "--max-single-order-value",
                    "100000",
                    "--max-total-order-value",
                    "200000",
                    "--output-dir",
                    str(tmp_path),
                ]
            )
    assert rc == 1
    audits = sorted(tmp_path.glob("live_approval_audit_*.json"))
    data = json.loads(audits[-1].read_text(encoding="utf-8"))
    assert data.get("status") == "LIVE_EXECUTION_BLOCKED_BY_SESSION"
    assert data.get("trading_session_open") is False
    assert data.get("trading_session_reason")
    assert session.post.call_count == 0 or not any(
        "order-cash" in str(c) for c in session.post.call_args_list
    )


def test_live_approve_session_open_proceeds_with_mocks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db = str(tmp_path / "sess.db")
    monkeypatch.setenv("DB_PATH", db)
    from deepsignal.storage.database import init_database, save_real_account_snapshot
    from deepsignal.live_trading.reconcile import ReconcileResult, write_latest_reconcile_state

    init_database(db)
    snap_now = datetime.now().isoformat(timespec="seconds")
    save_real_account_snapshot(
        db,
        snap_now,
        "kis",
        cash=1_000_000.0,
        withdrawable_cash=900_000.0,
        total_market_value=0.0,
        total_equity=1_000_000.0,
        raw_payload={"timestamp": snap_now, "positions": []},
    )
    report = tmp_path / "reconcile_seed.json"
    report.write_text(json.dumps({"success": True}), encoding="utf-8")
    write_latest_reconcile_state(
        report,
        ReconcileResult(success=True, matched=["005930"]),
        output_dir=tmp_path,
    )
    monkeypatch.setenv("KIS_APP_KEY", "k")
    monkeypatch.setenv("KIS_APP_SECRET", "s")
    monkeypatch.setenv("KIS_ACCOUNT_NO", "12345678")
    monkeypatch.setenv("KIS_ACCOUNT_PRODUCT_CODE", "01")
    monkeypatch.setenv("KIS_ENV", "live")
    p = _kr_plan(tmp_path)
    shared = _mock_session()

    open_result = TradingSessionResult(
        is_open=True,
        reason="regular trading session",
        market="KR",
        now=snap_now,
        timezone="Asia/Seoul",
        session_open="09:00",
        session_close="15:30",
    )

    with patch("deepsignal.live_trading.kis_broker.requests.Session", return_value=shared):
        with patch(
            "deepsignal.live_trading.trading_session.is_trading_session_open",
            return_value=open_result,
        ):
            rc = main_mod.main(
                [
                    "live-approve",
                    "--broker",
                    "kis",
                    "--plan",
                    str(p),
                    "--approved",
                    "--execute",
                    "--allow-live-env",
                    "--final-confirm",
                    "I_UNDERSTAND_REAL_ORDER",
                    "--allow-symbol",
                    "005930",
                    "--max-single-order-value",
                    "100000",
                    "--max-total-order-value",
                    "200000",
                    "--output-dir",
                    str(tmp_path),
                ]
            )
    assert rc == 0
    audits = sorted(tmp_path.glob("live_approval_audit_*.json"))
    data = json.loads(audits[-1].read_text(encoding="utf-8"))
    assert data.get("trading_session_open") is True
    urls = [c[0][0] for c in shared.post.call_args_list]
    assert sum(1 for u in urls if "order-cash" in u) == 1
