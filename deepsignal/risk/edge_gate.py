...
- DEEPSIGNAL_ALLOW_UNVERIFIED_CRYPTO_BUY (기본 false) — crypto_scalp_5m만의 비상 override.
  일반 edge gate off와 별도로, 코인 live BUY는 이 값이 true가 아니면 deploy=false에서 열리지 않는다.
"""
...
def allow_unverified_crypto_buy() -> bool:
    return os.environ.get("DEEPSIGNAL_ALLOW_UNVERIFIED_CRYPTO_BUY", "false").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def edge_gate_allows_buy(output_dir: str | Path, strategy: str) -> tuple[bool, str]:
    ...
    if strategy == "crypto_scalp_5m" and not st["deploy"] and not allow_unverified_crypto_buy():
        return (False, f"코인 엣지 하드차단: {st['reason']}")
