"""macOS launchd installer for kis-stream (user LaunchAgent)."""
...
KIS_STREAM_LABEL = "com.deepsignal.kis_stream"
KIS_STREAM_PLIST = f"{KIS_STREAM_LABEL}.plist"


@dataclass
class KisStreamLaunchdConfig:
    paper: bool = False          # True = 모의투자 WebSocket
    universe_size: int = 30      # 자동 유니버스 종목 수
...
def plist_path() -> Path:
    return launch_agents_dir() / KIS_STREAM_PLIST


def log_paths() -> tuple[Path, Path]:
    log_dir = Path.home() / ".deepsignal" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    return log_dir / "kis_stream.log", log_dir / "kis_stream.error.log"


def build_argv(launch_root: Path, cfg: KisStreamLaunchdConfig) -> list[str]:
    main_py = _main_py_for_launchd(launch_root)
    argv = [str(main_py), "kis-stream"]
    if cfg.paper:
        argv.append("--paper")
    else:
        argv.append("--live")
...
def install_kis_stream_launchd(
    ...
    *,
    ...
    payload = {
        ...
        "EnvironmentVariables": {
            ...
            "PYTHONUNBUFFERED": "1",
            "KIS_STREAM_AUTO_UNIVERSE": "true",
            "KIS_STREAM_UNIVERSE_SIZE": str(cfg.universe_size),
        },
    }
...
def uninstall_kis_stream_launchd(*, unload: bool = True, remove_plist: bool = True) -> dict[str, Any]:
    path = plist_path()
    unloaded = False
    msg = "skipped"
...
def kis_stream_launchd_status() -> dict[str, Any]:
    path = plist_path()
    if not path.is_file():
        return {"installed": False, "plist_path": path.as_posix()}
    proc = _run_launchctl(["print", f"{_gui_domain()}/{KIS_STREAM_LABEL}"])
