...
# 파생ETF 시드 차단 해제 — 계좌가 파생상품ETF(레버리지/인버스) 거래신청을
# 완료해 거래 가능해짐(2026-06-15). 레버리지/인버스도 정상 매수 대상.
_SEED_DERIV_ETF: set[str] = set()

# 거부 사유에 이 문자열이 포함되면 자동 블록.
# 주의: '파생ETF 미신청' 류는 제외 — 계좌가 이제 거래 가능하므로, 권한 반영
# 지연 등 일시 거부에 영구 차단되면 안 됨. 종목 자체가 거래불가한 경우만 학습.
_AUTO_BLOCK_REASON_KEYS = ("매매거래정지", "거래정지", "정리매매", "상장폐지")
